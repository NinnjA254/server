...
export async function getAllOrders(req, res) {
  try {
    const orders = await models.Order.find()
      ...
      .populate("itemsSold.productId", "name -_id");
    res.json({
      status: 200,
      message: "success",
      data: orders,
...
export async function fulfilOrder(req, res) {
  ...
  if (order.orderStatus === true) {
    ...
    for (let item of order.itemsSold) {
      ...
      if (!product)
        return res.json({
          message: `${product.name} was not found in the database`,
          status: 404,
          data: null,
        });
    ...
    if (outOfStock.length > 0) {
      //only one thing needs to be out of stock for order to not fulfill
      return res.json({
        message: `not enough ${outOfStock} in inventory`,
        status: 403,
        data: null,
